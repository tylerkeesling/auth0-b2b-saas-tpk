...
export default function UserLogs({ userId }: UserLogsProps) {
  ...
  // Initial load — no transition, skeleton handles loading state
  useEffect(() => {
    const loadInitial = async () => {
      await fetchLogs({ page: 0, type: 'all', fromDate: '', toDate: '' })
  ...
  // Subsequent page changes — uses transition for dimming overlay
  useEffect(() => {
    if (isInitialLoad) return
    startTransition(async () => {
      await fetchLogs({ page, type: selectedType, fromDate, toDate })
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page])
  ...
  const handleFilterChange = () => {
    setPage(0)
    startTransition(async () => {
      await fetchLogs({ page: 0, type: selectedType, fromDate, toDate })
    })
  }
    ...
    <Card>
      <CardContent className="space-y-4">
        ...
        {logs.length > 0 && (
          <>
            <div
              className={`rounded-md border transition-opacity ${isPending ? 'pointer-events-none opacity-50' : ''}`}
            >
              <Table>
                <TableHeader>
                  <TableRow>
